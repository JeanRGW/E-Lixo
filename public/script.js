...
async function getList() {
  try {
    if (list === null) { // Check if list has already been fetched
...
async function reservar(nome) {
  selectedItems.forEach(item => {
    ...
    fetch("http://localhost:3000/reservar", { method: "POST", headers: {"Content-Type": "application/json"}, body: JSON.stringify(data)}).then(response => {
      if(response.ok){
